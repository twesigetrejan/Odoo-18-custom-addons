...
class LoansProductMIS(models.Model):
    _name = 'sacco.loan.product.mis'
    _description = 'Loan Product MIS Integration'
    ...
    def _post_or_update_product(self, product, token):
        """Post or update a single loan product to the external system"""
        ...
        if not mis_record:
            ...
            _logger.info(f"Created new MIS record for loan product {product.name}: {mis_record.mongo_db_id}")
        ...
        try:
            _logger.info(f"Posting/Updating loan product to {api_url}: {product_data}")
            ...
            if response_data and 'docId' in response_data:
                ...
                if new_mongo_id != mongo_id:
                    ...
                    _logger.info(f"Updated mongo_db_id for loan product {product.name} to {new_mongo_id}")
                    
            mis_record.write({'last_sync_date': fields.Date.today()})
            return True
        except requests.RequestException as e:
            _logger.error(f"Failed to post/update loan product {product.name}: {str(e)}")
            return False

    def action_mass_sync_products(self, product_records):
        """Mass action to sync selected loan products"""
        if not product_records:
            raise ValidationError(_("No loan products selected for synchronization."))
        
        _logger.info(f"Starting mass loan product sync for {len(product_records)} products")
        ...
        for product in product_records:
            try:
                if self._post_or_update_product(product, token):
                    success_count += 1
                    self.env.cr.commit()
                    _logger.info(f"Successfully synced loan product {product.name}")
                else:
                    _logger.warning(f"Failed to sync loan product {product.name}")
            except Exception as e:
                self.env.cr.rollback()
                _logger.error(f"Error syncing loan product {product.name}: {str(e)}")

        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': _('Loan Product Sync'),
                'message': _('%d loan products processed successfully out of %d') % (success_count, len(product_records)),
    ...
    def sync_all_products(self):
        """Sync all loan products in batches"""
        _logger.info("Starting sync of all loan products")
        
        product_model = self.env['sacco.loan.type']
        offset = 0
        ...
        _logger.info("Completed sync of all loan products")