...
class SaccoPayingAccount(models.Model):
    _name = 'sacco.paying.account'
    _description = 'SACCO Paying Account Configuration'
    ...
    _sql_constraints = [
        ...
        ('account_id_unique', 'UNIQUE(account_id)', 'This Internal account is already linked to another paying account!'),
    ...
    def _check_account_requires_member(self):
        for record in self:
            if record.account_id.requires_member:
                raise ValidationError(_("You cannot select an account that requires member assignment for paying accounts."))
