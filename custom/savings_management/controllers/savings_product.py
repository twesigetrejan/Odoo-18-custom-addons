...
class SavingsProductController(http.Controller):
    @http.route('/api/v1/savings_products', type='http', auth='custom_auth', methods=['GET', 'OPTIONS'], csrf=False)
    def get_savings_products(self, **kwargs):
        # Handle preflight OPTIONS request
        if request.httprequest.method == 'OPTIONS':
            headers = {
                'Access-Control-Allow-Origin': '*',
                'Access-Control-Allow-Methods': 'GET, OPTIONS',
                'Access-Control-Allow-Headers': 'Authorization, Content-Type, Accept, X-AccountId',
        ...
        try:
            ...
            products = request.env['sacco.savings.product'].sudo().search([])
            ...
            _logger.exception(f"Unexpected error in /api/savings_products: {str(e)}")
