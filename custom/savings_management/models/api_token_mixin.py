...
class APITokenMixin(models.AbstractModel):
    ...
    def _get_authentication_token(self):
        ...
        # If system is not configured, return None to trigger local update
        if not username or not password:
            _logger.info("External system not configured, skipping authentication.")
            return None, None

        token, expiry, account_id = self._get_stored_token_info()
        _logger.info(f"Stored token obtained")
        ...
        try:
            ...
            if not account_id:
                ...
                raise UserError(_("Login failed due to poor configuration"))
    ...
    def _get_request_headers(self):
        ...
        if not token or not account_id:
            _logger.error("Missing authentication credentials. Please log in again.")
    ...
    def _handle_external_action(self, action_callback, local_update_callback=None, success_message="Action completed successfully", local_message="Action completed locally, will sync later"):
        """
        ...
        # Check if system is configured to connect to external system
        if not username or not password:
            # Perform local update if callback is provided
            if local_update_callback:
                try:
                    local_update_callback()
                except Exception as e:
                    _logger.error(f"Local update failed: {str(e)}")
                    return self._show_notification('Error', 'Local update failed, Please contact support', 'danger')
            
            # Mark as out of sync if applicable (e.g, withdrawals)
            if hasattr(self, 'in_sync'):
                self.write({'in_sync': False})
        ...
        try:
            ...
            if result and isinstance(result, dict) and result.get('params', {}).get('type') == 'success':
                if hasattr(self, 'in_sync'):
                    self.write({'in_sync': True})
                return self._show_notification('Success', success_message, 'success', next_action={'type': 'ir.actions.client', 'tag': 'reload'})
            else:
                if hasattr(self, 'in_sync'):
                    self.write({'in_sync': False})
                error_message = result.get('params', {}).get('message', 'External action failed')
                return self._show_notification('Error', error_message, 'danger')
        except Exception as e:
            _logger.error(f"External action failed: {str(e)}")
            if hasattr(self, 'in_sync'):
                self.write({'in_sync': False})
            return self._show_notification('Error', 'External action failed, Please contact support', 'danger')

    def _show_notification(self, title, message, type='info', next_action=None):
        """Helper method to show notifications with optional next action."""
        notification = {
        ...
        if next_action:
            notification['params']['next'] = next_action
        return notification
