...
class SavingsProductMIS(models.Model):
    _name = 'sacco.savings.product.mis'
    _description = 'SACCO Savings Product MIS Integration'
    ...
    def _post_or_update_product(self, product, token):
        ...
        if not mis_record:
            ...
            _logger.info(f"Created new MIS record for product {product.name}: {mis_record.mongo_db_id}")
        ...
        try:
            _logger.info(f"Posting/Updating product to {api_url}: {product_data}")
            ...
            if response_data and 'docId' in response_data:
                ...
                if new_mongo_id != mongo_id:
                    ...
                    _logger.info(f"Updated mongo_db_id for product {product.name} to {new_mongo_id}")
                    
            mis_record.write({'last_sync_date': fields.Date.today()})
            return True
        except requests.RequestException as e:
            _logger.error(f"Failed to post/update product {product.name}: {str(e)}")
            return False

    def action_mass_sync_products(self, product_records):
        """Mass action to sync selected savings products"""
        if not product_records:
            raise ValidationError(_("No products selected for synchronization."))
        
        _logger.info(f"Starting mass product sync for {len(product_records)} products")
        ...
        for product in product_records:
            try:
                if self._post_or_update_product(product, token):
                    success_count += 1
                    self.env.cr.commit()
                    _logger.info(f"Successfully synced product {product.name}")
                else:
                    _logger.warning(f"Failed to sync product {product.name}")
    ...
    def sync_all_products(self):
        ...
        _logger.info("Starting sync of all savings products")
        
        product_model = self.env['sacco.savings.product']
        offset = 0
        ...
        _logger.info("Completed sync of all savings products")