...
class TheadIdImportWizard(models.TransientModel):
    ...
    def action_import(self):
        ...
        required_columns = ['ACODE', 'AmtUSD', 'AMOUNT', 'VDATE', 'TTYPE', 'PAYDET', 'MemberID', 'ProductID']
            ...
            try:
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        # If no account is found or MemberID/ProductID is missing, try finding account by ACODE
                        if not account:
                            ...
                            # If still no account found, use the default collection account
                            if not account:
                                account = self.collection_account_id
                            member = False  # Don't set member when using ACODE or default account
                        ...
                        if amt_usd != 0.0:  # Check if AmtUSD is not empty or zero
                            # Find the USD currency
                            ...
                            # Amount in currency must be positive when debited, negative when credited
                            if ttype == 'D':
                                line_vals.update({
                                    'debit': amount,
                                    'credit': 0.0,
                            ...
                            # If AmtUSD is not provided, proceed without currency conversion
                            if ttype == 'D':
                                line_vals.update({
                                    'debit': amount,
                                    'credit': 0.0,
